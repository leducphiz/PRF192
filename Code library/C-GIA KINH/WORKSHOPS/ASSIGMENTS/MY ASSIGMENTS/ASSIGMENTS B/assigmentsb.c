...
void input(book *p,int *n){
  ...
  for(i=0; i<*n;i++){
    ...
    printf("Enter title: "); scanf("%s", (p+i)->title);
...
void editBook(book *p, int *n){
  ...
  if(k!=-1){
    ...
    printf("Enter title: "); scanf("%s", (p+k)->title);
...
void writeFile(FILE *fptr,book *p, int *n){
  int i;
  double totalPrice=0;
  fprintf(fptr, "code \t\t\ttitle \t\tQuantity \t\tPrice \t\tValue\n");
  for(i=0;i<*n;i++){
    fprintf(fptr, "%s %15s %10d %15.1lf %12.1lf\n",(p+i)->code, (p+i)->title, (p+i)->quantity, (p+i)->price, (p+i)->value);
    totalPrice += (p+i)->value;
  }
  fprintf(fptr, "\t\t\t\t\t\t\t\t\t\t\t\t\t\tTotal price: %0.1lf\n", totalPrice);
}
